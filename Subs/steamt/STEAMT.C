/* STEAMT.C   M. W. Klotz   9/03

Find the temperature of boiling water at elevated pressure using the
...
dbl c0=273.18;			/* convert Celsius to Kelvin */
dbl deltah=40.7;		// water heat of vaporization (kJ/mol)
dbl r=8.3143e-3;		// gas constant (kJ/mol/degK)
dbl atm=14.696;			// atmospheric pressure (psi)
dbl p,t;			// pressure, temperature
...
vin ("Boiler Gage Pressure",&p,"lf",0.,"psi");
t=1./(-log((p+atm)/atm)*(r/deltah)+1./(100.0+c0))-c0;	//degC
printf ("Water temperature = %.0lf degC = %.0lf degF\n",t,1.8*t+32);
