/* STEAMHP.C   M. W. Klotz   5/02

Calculate approximate horsepower for a steam engine.
Modeled after a program by Don Nichols.
...
dbl hp1;	   /* hp on primary piston end */
dbl hp2;	   /* hp on secondary (con-rod) end */
dbl pressure;	   /* psi */
dbl stroke;	   /* stroke (ft) */
dbl diameter;	   /* diameter (in) */
dbl roddia;	   /* diameter of con rod (in) */
dbl area1;	   /* pirmary piston area */
dbl area2;	   /* secondary piston area */
dbl spm;	   /* strokes/minute */
...
puts ("STEAM ENGINE HORSEPOWER CALCULATION\n");

vin ("Steam pressure",&pressure,"lf",60.,"psi");
vin ("Stroke",&stroke,"lf",1.5,"in");
vin ("Strokes per minute",&spm,"lf",1000.,"");
vin ("Piston diameter",&diameter,"lf",1.5,"in");
puts ("If single acting, enter zero for the following:");
vin ("Con rod diameter for double acting engine",&roddia,"lf",0.25,"in");

stroke/=12.;
area1=0.25*PI*diameter*diameter;
hp1=pressure*stroke*area1*spm/33000;

if (roddia)
{
area2=area1-0.25*PI*roddia*roddia;
hp2=pressure*stroke*area2*spm/33000;
}
else hp2=0;

printf ("\nPrimary piston end hp produced = %.4lf hp\n",hp1);
printf ("Secondary piston end hp produced = %.4lf hp\n",hp2);
printf ("Total horsepower produced = %.4lf hp\n",hp1+hp2);
