...
{
int i;
dbl d,h,k,x,y,m,n,r,q;

/* -------------------------- initializations ------------------------------ */
textscreen ();
/* ------------------------------------------------------------------------- */

puts ("KEYWAY CALCULATIONS\n");
...
print;
vin ("Shaft diameter",&d,"lf",1.0,"in");
vin ("Key width",&k,"lf",0.25,"in");
vin ("Key height",&h,"lf",0.3,"in");

x=sqrt(d*d-k*k); y=0.5*(d-x); m=d-(y+0.5*h); n=m+h; q=d-m; r=h-q;

print;
printf ("Shaft diameter (D)         = %.4lf in\n",d);
printf ("Key width (K)              = %.4lf in\n",k);
printf ("Key height (H)             = %.4lf in\n",h);
printf ("Keyseat depth (M)          = %.4lf in\n",m);
printf ("Keyway depth (N)           = %.4lf in\n",n);
printf ("Depth of cut in shaft (Q)  = %.4lf in\n",q);
printf ("Depth of cut in hub (R)    = %.4lf in\n",r);
print;
