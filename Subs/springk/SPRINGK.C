/* SPRINGK.C   M. W. Klotz   8/03

Calculation of spring constant after a formula provided by
...
puts ("Dave Baker's SPRING CONSTANT CALCULATION\n");

vin ("Outside diameter of spring",&od,"lf",1.34,"in");
od*=25.4;		//od in mm
vin ("Wire diameter",&d,"lf",0.157,"in");
d*=25.4;		// d in mm
vin ("Compressed length of spring",&l,"lf",1.1,"in");
l*=25.4;		// l in mm

D=od-d;			// mean diameter (mm)
N=l/d-1.7;		// number of working coils

k=pow(d,4.)/pow(D,3.)/N*6.e4;

printf ("\nSpring constant = %g lb/in\n",k);
