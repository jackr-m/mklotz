/* TAPER.C   M. W. Klotz   2/99

Cylindrical taper calculations

Adapted from a GWBASIC program written by
Battle Vaughan (wizone@herald.infi.net)
...
{
int i,j,k;
dbl dl,ds,length,taper,angle,setover,phi;

/* -------------------------- initializations ------------------------------ */
textscreen ();
/* ------------------------------------------------------------------------- */

puts ("CYLINDRICAL TAPER CALCULATIONS\n");

puts ("\nInput whatever data you know; press return if not known.");
puts ("You must enter three data items to obtain a solution.\n");
k=0; angle=0.; phi=0.;
vin ("Small diameter",&ds,"lf",0.,"in"); if (ds) k++;
vin ("Large diameter",&dl,"lf",0.,"in"); if (dl) k++;
vin ("Length",&length,"lf",0.,"in"); if (length) k++;
if (k == 3) goto doit;
vin ("Taper",&taper,"lf",0.,"in/in"); if (taper) k++;
if (k == 3) goto doit;
if (taper == 0.)
{
vin ("Included angle",&angle,"lf",0.,"deg"); if (angle) k++;
if (k == 3) goto doit;
}
if (taper == 0. && angle == 0.)
{
vin ("Taper angle",&phi,"lf",0.,"deg"); if (phi) k++;
if (k == 3) goto doit;
}
...
doit:;
if (phi != 0.) angle=2.*phi;
if (taper == 0. && angle != 0.) {phi=0.5*angle; taper=2.0*TAND(phi);}
if (taper != 0. && angle == 0.) {phi=ATAND(0.5*taper); angle=2.*phi;}

if (taper)
{
if (length == 0.) {length=(dl-ds)/taper; goto disp;}
if (dl == 0.) {dl=taper*length+ds; goto disp;}
if (ds == 0.) {ds=dl-taper*length; goto disp;}
}
...
setover=0.5*(dl-ds);
print;
printf ("Small diameter = %.4lf in\n",ds);
printf ("Large diameter = %.4lf in\n",dl);
printf ("Length         = %.4lf in\n",length);
printf ("Taper          = %.4lf in/in\n",taper);
printf ("Taper          = %.4lf in/ft\n",12*taper);
printf ("Included angle = %.4lf deg\n",angle);
printf ("Taper angle    = %.4lf deg\n",phi);
printf ("Lathe setover  = %.4lf in\n",setover);
