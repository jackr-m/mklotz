/* CSK.C   M. W. Klotz  2/99

Countersink computations
...
{
int i,j,k;
dbl theta,dh,dcsk,a,b,tz;

/* -------------------------- initializations ------------------------------ */
textscreen ();
/* ------------------------------------------------------------------------- */

puts ("COUNTERSINK COMPUTATIONS\n");

vin ("Included angle of countersink",&theta,"lf",60.,"deg");
tz=TAND(0.5*theta);
agin:
vin ("Drilled hole diameter",&dh,"lf",0.5,"in");
vin ("Desired countersink diameter",&dcsk,"lf",0.6,"in");
if (dcsk <= dh)
{
puts ("Whoops, csk diameter smaller than drilled hole diameter");
beep; goto agin;
}
a=0.5*(dcsk-dh)/tz;
printf ("\nDepth from initial contact with drilled hole = %.4lf in",a);
b=0.5*dcsk/tz;
printf ("\nDepth from surface of work = %.4lf in",b);
